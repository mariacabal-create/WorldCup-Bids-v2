...
export function MatchCard({
  ...
  return (
    <article className="bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden shadow-xl hover:border-yellow-500 transition-all">
      <div className="px-5 pt-5">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            <div className="text-3xl">{FLAGS[match.home] ?? "⚽"}</div>
            <div className="text-xl font-bold text-white">{match.home}</div>
          </div>

          <div className="flex items-center gap-3">
            <div className="text-3xl">{FLAGS[match.away] ?? "⚽"}</div>
        ...
        <div className="mt-5 rounded-2xl bg-black/30 border border-zinc-800 p-4 space-y-3">
          <div className="flex justify-between text-sm">
            <span className="text-zinc-400">🎯 Marcadores tomados</span>
          ...
          <div className="flex justify-between text-sm">
            <span className="text-zinc-400">🔥 Pujas realizadas</span>
            <span className="text-white font-semibold">
              {summary?.bid_count ?? 0}
            </span>
          </div>
      ...
      <div className="border-t border-zinc-800 mt-5 px-5 py-4 flex items-center justify-between gap-3">
        <Countdown kickoff={match.kickoff} />

        <button
          onClick={() => onOpenBid(match)}
          disabled={closed}
          className="bg-yellow-500 hover:bg-yellow-400 text-black font-bold px-5 py-3 rounded-2xl transition-all disabled:bg-zinc-700 disabled:text-zinc-400 disabled:cursor-not-allowed"
        >
          {closed ? "Cerrada" : topBid ? "Ver / pujar" : "Pujar"}
        </button>
      </div>
    </article>
  );
